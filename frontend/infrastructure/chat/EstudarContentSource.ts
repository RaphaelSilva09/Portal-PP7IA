...
export class EstudarContentSource implements ContentSource {
    readonly sourceType = "estudar";

    async fetchAll(): Promise<SourceItem[]> {
        const { rows } = await pool.query<Row>(
            `SELECT id, title, html_path FROM public.estudar WHERE html_path IS NOT NULL`,
        );
        ...
        for (const row of rows) {
            ...
            try {
                ...
                console.warn(`EstudarContentSource: read failed for ${row.html_path}: ${code ?? (err as Error).message}`);
                continue;
            }
