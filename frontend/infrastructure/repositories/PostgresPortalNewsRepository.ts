/**
 * PostgresPortalNewsRepository (Infrastructure Layer)
 *
 ...
 */
interface PostgresPortalNewsRow {
    id: string;
    title: string;
 ...
 */
export class PostgresPortalNewsRepository implements IPortalNewsRepository {
    /**
    ...
    async getActive(): Promise<PortalNewsItem[]> {
        try {
            ...
            return rows
                ...
                .map(row => this.mapToEntity(row as unknown as PostgresPortalNewsRow));
        } catch (err) {
     ...
     */
    private isValidRow(row: unknown): row is PostgresPortalNewsRow {
     ...
     */
    private mapToEntity(row: PostgresPortalNewsRow): PortalNewsItem {
