/**
 * PostgresHomeDatesRepository (Infrastructure Layer)
 *
 ...
 */
export class PostgresHomeDatesRepository {
    /**
    ...
    async getLatestDates(): Promise<HomePageDates> {
        try {
            ...
            if (!rows || rows.length === 0) {
                console.warn("[PostgresHomeDatesRepository] No data returned from mv_home_latest_dates");
            ...
            console.error("[PostgresHomeDatesRepository] Unexpected error fetching home dates:", err);
            return this.emptyDates();
        }
    }
