/**
 * PostgresNewsletterRepository (Infrastructure Layer)
 *
 ...
 */
export class PostgresNewsletterRepository implements INewsletterRepository {
    /**
    ...
    async getAll(): Promise<Newsletter[]> {
        try {
            ...
            const items = (rows as unknown[])
                .filter((row): row is PostgresNewsletterRow => this.isValidRow(row))
     ...
     */
    private isValidRow(row: unknown): row is PostgresNewsletterRow {
     ...
     */
    private mapToEntity(row: PostgresNewsletterRow): Newsletter {
