...
export async function POST(request: NextRequest) {
    ...
    // 4. Retrieve — hybrid direct match (mini-livro only) OR parallel semantic fan-out
    let allChunks: RetrievedChunk[];

    const directMatch = lastUser.content.match(/\b(?:mini[\s-]?livro|ml)[\s-]?0*(\d{1,2})\b/i);
    if (directMatch) {
        // Targeted query — only this mini-livro; skip all other sources (including meta)
        const directChunks = await chunkRepo.findBySlug("mini_livro", String(directMatch[1]).padStart(3, "0"));
        allChunks = directChunks;
    } else {
        // Parallel semantic search across all registered sources
        const results = await Promise.all(
            RAG_SOURCES.map(src =>
                chunkRepo.searchSimilar({
    ...
    // 6. Dedupe citations: one per (source_type, slug, chapter).
    //    meta_summary chunks use parent fields for the dedup key.
    const CITATION_CAP = 8;
    type Cit = ReturnType<typeof citationFromMetadata>;

    const chapterKeyOf = (c: RetrievedChunk): string => {
        if (c.source_type === "meta_summary") {
            const parentType = c.metadata.parent_source_type ?? "meta";
            const parentSlug = c.metadata.parent_slug ?? c.metadata.slug;
            return `${parentType}|${parentSlug}|meta`;
        }
        return `${c.source_type}|${c.metadata.slug}|${(c.metadata.heading_path ?? []).slice(0, 2).join("|")}`;
    };

    const bestPerKey = new Map<string, Cit>();
    for (const c of citableCandidates) {
        const key = chapterKeyOf(c);
        const cit = citationFromMetadata(c.metadata, c.similarity, c.source_type);
        const existing = bestPerKey.get(key);
        if (!existing || cit.similarity > existing.similarity) {
            bestPerKey.set(key, {
                ...cit,
                heading_path: cit.heading_path.slice(0, 2),
            });
        }
    }
    ...
    const chunkToCitationIdx = citableCandidates.map(c => keyToCitationIdx.get(chapterKeyOf(c)) ?? 0);
    const usedChunks = citableCandidates.filter((_, i) => chunkToCitationIdx[i] > 0);
