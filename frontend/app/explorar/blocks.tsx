...
const C = {
    newsletter: { color: "var(--block-newsletter)", soft: "var(--block-newsletter-soft)", num: "01", label: "Newsletter",          code: "PPNEWS"  },
    reportagem: { color: "var(--block-reportagem)", soft: "var(--block-reportagem-soft)", num: "02", label: "Reportagem da Semana", code: "PPREP"  },
    radar:      { color: "var(--block-radar)",      soft: "var(--block-radar-soft)",      num: "03", label: "Radar",               code: "PPRADAR" },
    livro:      { color: "var(--block-livro)",      soft: "var(--block-livro-soft)",      num: "04", label: "Enquanto é Tempo",    code: "PPLIVRO" },
    biblioteca: { color: "var(--block-biblioteca)", soft: "var(--block-biblioteca-soft)", num: "05", label: "Biblioteca",          code: "PPBIB"   },
    estudar:    { color: "var(--block-estudar)",    soft: "var(--block-estudar-soft)",    num: "06", label: "Estudar",             code: "PPEST"   },
} as const;
            ...
            <div className="p-8 md:p-12">
                <div className="relative z-10 flex flex-wrap items-center gap-3">
                    <span
                        ...
                        style={{ backgroundColor: block.soft, color: block.color }}
                    >
...
const LIVRO_PARTS = [
    { order: 1 as const, romanLabel: "I",   partLabel: "Parte I",   subtitle: "Liderança Híbrida",     anchorId: "parte-i"   },
    { order: 2 as const, romanLabel: "II",  partLabel: "Parte II",  subtitle: "A Coragem de Executar", anchorId: "parte-ii"  },
    { order: 3 as const, romanLabel: "III", partLabel: "Parte III", subtitle: "O Que Fica",            anchorId: "parte-iii" },
];

function BookHeroCard({ book, block, miniLivros, sortedEbooks }: {
    book: Book;
    block: BlockCfg;
    miniLivros: Array<{ partOrder: number }>;
    sortedEbooks: Array<{ description: string | null }>;
}) {
    const hasIntro = Boolean(book.introHtmlPath);
    const hasCoverPdf = Boolean(book.coverPdfPath);
    return (
        <article
            className="grid grid-cols-1 items-center gap-8 overflow-hidden rounded-3xl border border-border p-8 md:grid-cols-[200px_1fr] md:p-10"
            style={{ background: `linear-gradient(160deg, ${block.soft} 0%, var(--card) 70%)` }}
        >
            {book.coverImagePath && (
                <div className="relative mx-auto w-[180px] md:w-full">
                    <div
                        className="absolute inset-0 translate-x-2 translate-y-3 rounded-lg opacity-30 blur-xl"
                        style={{ backgroundColor: block.color }}
                        aria-hidden="true"
                    />
                    <img
            ...
            <div>
                <span
                    ...
                    style={{ backgroundColor: block.color, color: "var(--background)" }}
                >
                    {book.badgeText ?? "Em desenvolvimento"}
                </span>
                <h2 className="mt-4 font-serif text-4xl leading-[1.1] tracking-[-0.02em] text-ink md:text-5xl">
                    {book.title}
                </h2>
                {book.description && (
                    <p className="mt-3 max-w-xl text-muted-foreground">{book.description}</p>
                )}
                <div className="mt-5 flex flex-wrap gap-2">
                    {LIVRO_PARTS.map(part => {
                        const count = miniLivros.filter(ml => ml.partOrder === part.order).length;
                        const ebookDesc = sortedEbooks[part.order - 1]?.description ?? null;
                        return (
                            <a
                                key={part.order}
                                href={`#${part.anchorId}`}
                                className="inline-flex items-center gap-2 rounded-full border border-border bg-card px-3 py-1.5 text-xs font-medium text-muted-foreground transition-all hover:-translate-y-0.5 hover:text-foreground"
                            >
                                <span className="font-serif text-[11px] tracking-wide" style={{ color: block.color }}>
                                    {part.romanLabel}
                                </span>
                                {ebookDesc ? part.subtitle : part.subtitle}
                                {count > 0 && (
                                    <span
...
function PartEbookCard({ ebook, block }: { ebook: Ebook; block: BlockCfg }) {
    const hasIntro = ebook.htmlAvailable;
    const hasCoverPdf = ebook.coverPdfAvailable;
    return (
        <div
            className="grid grid-cols-1 items-center gap-6 overflow-hidden rounded-3xl border border-border p-6 md:grid-cols-[140px_1fr] md:gap-8 md:p-8"
            style={{ background: `linear-gradient(160deg, ${block.soft} 0%, var(--card) 75%)` }}
        >
...
function MiniLivroListCard({ item, block }: { item: Item; block: BlockCfg }) {
    const href = item.htmlPath;
    return (
        <li>
            <div className="group relative flex items-stretch gap-4 rounded-2xl border border-border bg-card p-5 transition-all hover:-translate-y-0.5 hover:shadow-[var(--shadow-elevated)]">
                ...
                <div className="flex-1">
                    <div className="text-[10px] uppercase tracking-[0.22em]" style={{ color: block.color }}>
                        Mini-Livro #{item.formattedNumber}
                    </div>
                    {href ? (
...
function BookExtraSection({
    ...
    return (
        <div className="space-y-5">
            <div className="flex items-end gap-4 border-b border-border py-4">
                <div className="flex-1">
                    <div className="mb-1 text-[10px] uppercase tracking-[0.22em]" style={{ color: block.color }}>
                        Enquanto é Tempo
                    </div>
        ...
        <section className="py-12">
            <div className={portalContentClass}>
                ...
                <div className="space-y-14">
                    {/* Livro principal (singleton) */}
                    {book && (
                        <BookHeroCard
                            book={book}
                            block={block}
                            miniLivros={miniLivros}
                            sortedEbooks={sortedEbooks}
                        />
                    )}
                        ...
                        {LIVRO_PARTS.map(part => {
                            ...
                            return (
                                <section key={part.order} id={part.anchorId} className="scroll-mt-32 space-y-6">
                                    ...
                                    {ebook && <PartEbookCard ebook={ebook} block={block} />}
