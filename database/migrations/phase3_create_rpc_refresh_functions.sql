...
COMMENT ON FUNCTION refresh_mv_home_latest_dates() IS 
  'Refreshes home latest dates materialized view. Admin-only. ' ||
  'Call after publishing/updating content in: newsletters, mini_livros, ' ||
  'biblioteca, especial_semana, radar_oportunidades, estudar. ' ||
  'Usage: await supabase.rpc(''refresh_mv_home_latest_dates'');';
...
COMMENT ON FUNCTION refresh_mv_admin_dashboard_stats() IS 
  'Refreshes admin dashboard stats materialized view. Admin-only. ' ||
  'Call after any content/user changes that affect dashboard statistics. ' ||
  'Usage: await supabase.rpc(''refresh_mv_admin_dashboard_stats'');';
