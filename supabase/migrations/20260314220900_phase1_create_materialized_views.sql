...
COMMENT ON MATERIALIZED VIEW mv_home_latest_dates IS 'Consolidates latest created_at from 6 content tables. Refresh via RPC: SELECT refresh_mv_home_latest_dates();';
...
COMMENT ON MATERIALIZED VIEW mv_admin_dashboard_stats IS 'Admin-only dashboard statistics. Consolidates 14+ queries into 1. Refresh via RPC: SELECT refresh_mv_admin_dashboard_stats();';
